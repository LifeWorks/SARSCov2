...
extract_gene_set <- function(geneset, setlist) {
  ...
  return(list(names=names, desc=desc, size=size, matrix=mat))
}
...
enrichment_in_groups <- function(geneset, targets=NULL, background=NULL, method="fishers", minsize=5,
  ...
  resultp = c()
	...
	colnames(results) = c("in_path", "out_path", "in_back", "out_back", "foldx", "pvalue", "Adjusted_pvalue", "Signed_AdjP")
  ...
  if (method == "ks") colnames(results)[c(2,4)] = c("MeanPath", "Zscore")
    ...
    if (method == "fishers") {
      ...
      if (in_path > minsize) {
        ...
        if (is.null(mapping_column)) {
          in_rank = rank(backlist)[grouplist[which(grouplist %in% names(background))]]
        } else {
          in_rank = rank(backlist)[which(background[,mapping_column] %in% grouplist)]
        }
        ...
        results[thisname,] = c(in_path, mean(in_group, na.rm=T), in_back, zscore, foldx, p.value, NA, NA)
      }
    }
  }
...
enrichment_in_relationships <- function(geneset, relationships, idmap=NA, tag=NA) {
...
correlation_enrichment <- function(geneset, abundance, mapping_column=NA, tag=NA) {
  ...
  return(enrichment_in_relationships(geneset, allgenes_cor, idmap=map, tag=tag))
}
...
enrichment_in_abundance <- function(geneset, abundance, mapping_column=NULL, abundance_column=NULL, 
  ...
  if (!is.null(mapping_column)) groupnames = unique(abundance[,mapping_column])
    ...
    if (!is.null(mapping_column)) {
      ...
      ingroup = abundance[which(abundance[,mapping_column] %in% ingroupnames), abundance_column]
      
      if (!is.null(sample_comparison)) outgroup = abundance[which(abundance[,mapping_column] %in% ingroupnames), sample_comparison]
      ...
      # I changed this to make it easier to use- may break old code!
		  ...
		  ingroup = unlist(abundance[grouplist[which(grouplist %in% rownames(abundance))], abundance_column])
      
      if (!is.null(sample_comparison)) outgroup = abundance[which(rownames(abundance) %in% grouplist), sample_comparison]
      else outgroup = abundance[which(!rownames(abundance) %in% grouplist), abundance_column]
		}
