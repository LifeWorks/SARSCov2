...
for (ztype in ztypes) {
  infile <- sprintf("./biggerTrans-%s.csv", ztype)
  infileNoSars2 <- sprintf("./biggerTransNoSars2-%s.csv", ztype)
  outfile <- sprintf("./biggerTransSars2Effect-%s.csv", ztype)
  if (file.exists(infile)) {
    genelist <- read.csv(infile, header = TRUE, row.names = 1)
    genelist2 <- read.csv(infileNoSars2, header = TRUE, row.names = 1)
    allGenenames <- rownames(genelist)
    allGenenames2 <- rownames(genelist2)
    ...
    for (j in seq(1,length(ss))) {
      ...
      ranked <- cbind(rankedNoSars2, dfsorted)
    }
    write.csv(ranked, outfile)
  }
}
